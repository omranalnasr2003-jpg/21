...
function RulesModal({ onClose }) {
  const rules = [
    ...
    ['Ziel','Erster mit dem Punkteziel gewinnt'],
  ]
  return (
      ...
      <div style={{ width:'100%', maxWidth:360, background:C.surface, borderRadius:18, padding:24, border:`1px solid ${C.borderHi}` }}>
        ...
        {tab==='join' && (
          <>
            ...
            <Btn disabled={loading} onClick={async()=>{ if(!joinName.trim()){setErr('Bitte Namen eingeben');return} if(!joinCode.trim()){setErr('Bitte Code eingeben');return} setLoading(true); await onJoinGame(joinName.trim(),joinCode.trim(),setErr); setLoading(false) }}>{loading?'…':t.join}</Btn>
          </>
        )}
      </div>
    ...
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:20 }}>
      ...
      <div style={{ width:'100%', maxWidth:380, background:C.surface, borderRadius:18, padding:26, border:`1px solid ${C.borderHi}` }}>
        <div style={{ textAlign:'center', marginBottom:22 }}>
          ...
          <button onClick={copy} style={{ padding:'6px 20px', borderRadius:8, border:`1px solid ${C.border}`, background:'transparent', color:copied?C.greenLt:C.textSub, cursor:'pointer', fontSize:13 }}>{copied?'✓ Kopiert!':'Code kopieren'}</button>
        </div>
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.86)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:150, padding:16 }}>
      <div style={{ background:C.surface, borderRadius:18, padding:24, border:`1.5px solid ${C.gold}`, maxWidth:400, width:'100%', maxHeight:'88vh', overflowY:'auto' }}>
        ...
        <div style={{ color:C.textSub, fontSize:13, marginBottom:18 }}>Mehrere 11er möglich — welche nimmst du?</div>
        {combos.map((combo,i)=>(
...
// ─── CARD with animation styles ───────────────────────────────────────────────
function AnimCard({ card, glow, fade, landing, small, disabled, onClick }) {
  const red = card.suit==='♥'||card.suit==='♦'
  const w = small?44:62, h = small?64:90
  let bg = 'linear-gradient(150deg,#fff,#f5ede0)'
  let border = '#c8b898'
  let shadow = '0 3px 12px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.9)'
  let textColor = red?'#991b1b':'#1e293b'
  let anim = ''

  if (glow) {
    bg = 'linear-gradient(150deg,#052010,#031208)'
    border = '#34d399'
    shadow = '0 0 0 3px rgba(52,211,153,.45),0 4px 20px rgba(52,211,153,.3)'
    textColor = red?'#fca5a5':'#6ee7b7'
  }

  const style = {
    width:w, height:h, borderRadius:8, flexShrink:0,
    background:bg, border:`1.5px solid ${border}`,
    boxShadow:shadow, cursor:disabled?'default':'pointer',
    display:'flex', flexDirection:'column', justifyContent:'space-between',
    padding:'4px 6px', userSelect:'none', position:'relative', overflow:'hidden',
    transition: glow ? 'background .15s, box-shadow .15s, border .15s' : 'none',
    opacity: fade ? 0 : 1,
    transform: fade ? 'scale(0.3) translateY(-30px)' : landing ? 'scale(1.08) translateY(-4px)' : 'scale(1)',
    transitionProperty: fade ? 'opacity, transform' : landing ? 'transform' : 'none',
    transitionDuration: fade ? '0.4s' : landing ? '0.2s' : '0s',
    transitionTimingFunction: fade ? 'ease-in' : 'ease-out',
  }

  return (
    <div style={style} onClick={onClick}>
      <div style={{ position:'absolute', inset:0, background:'linear-gradient(135deg,rgba(255,255,255,.18) 0%,transparent 50%)', pointerEvents:'none', borderRadius:7 }}/>
      <div style={{ fontSize:small?10:15, fontWeight:700, color:textColor, lineHeight:1.1, position:'relative' }}>{card.rank}<br/><span style={{ fontSize:small?9:12 }}>{card.suit}</span></div>
      <div style={{ fontSize:small?10:15, fontWeight:700, color:textColor, lineHeight:1.1, alignSelf:'flex-end', transform:'rotate(180deg)', position:'relative' }}>{card.rank}<br/><span style={{ fontSize:small?9:12 }}>{card.suit}</span></div>
    </div>
  )
}
...
function Game({ gs, myIdx, onPlayCommit, t, onLeave, pendingGs, onAnimDone }) {
  ...
  const [glowIds,     setGlowIds]     = useState([])  // glowing green
  const [fadeIds,     setFadeIds]     = useState([])  // fading out
  ...
  const canPlay = currentPlayer === myIdx && phase === 'playing' && !busyRef.current && !comboMenu

  const runAnim = useCallback((idsToGlow, card, combo) => {
    setTimeout(() => {
      setGlowIds(idsToGlow)
      setTimeout(() => {
        setFadeIds(idsToGlow)
        setGlowIds([])
        setTimeout(() => {
          setPhase1Cards([])
          setFadeIds([])
  ...
  const handleCardClick = useCallback((card) => {
    ...
    const tableWith = [...tableRef.current, card]
    // Show card on table immediately
    setPhase1Cards(tableWith)

    if (isJack(card.rank)) {
      const ids = tableWith.filter(c=>c.rank!=='Q'&&c.rank!=='K').map(c=>c.id)
      runAnim(ids, card, [])
      return
    }
    if (card.rank==='Q') {
      const other = tableWith.filter(c=>c.rank==='Q'&&c.id!==card.id)
      if (other.length) { runAnim([card.id,other[0].id], card, []); return }
      setTimeout(()=>{ setPhase1Cards([]); onPlayCommit(card,[]) }, 400)
      return
    }
    if (card.rank==='K') {
    ...
    if (!combos.length) { setTimeout(()=>{ setPhase1Cards([]); onPlayCommit(card,[]) }, 400); return }
    if (combos.length===1) { runAnim([card.id,...combos[0].map(c=>c.id)], card, combos[0]); return }
    busyRef.current = false
    setComboMenu({ card, combos })
  }, [canPlay, runAnim, onPlayCommit])

  const handlePick = useCallback((card, combo) => {
    busyRef.current = true
    setComboMenu(null)
    const ids = [card.id,...combo.map(c=>c.id)]
    runAnim(ids, card, combo)
  }, [runAnim])

  // What to show on table
  const displayTable = phase1Cards.length > 0 ? phase1Cards : table
    ...
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', padding:'10px 8px', overflowX:'hidden' }}>
        ...
        return (
          <div key={pi} style={{ width:'100%', maxWidth:560, background:C.surface, borderRadius:12, padding:'8px 12px', marginBottom:6, border:pi===currentPlayer?`1.5px solid ${C.blueLt}`:`1px solid ${C.border}`, transition:'border .3s' }}>
            ...
            <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
              {(hands[pi]||[]).map((_,i)=><CardBack key={i} small/>)}
            </div>
          </div>
        )
      })}
      ...
      <div style={{ width:'100%', maxWidth:560, background:C.surface, borderRadius:14, padding:'10px 12px', marginBottom:6, border:`1px solid ${C.border}`, minHeight:130, position:'relative' }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:7, minHeight:84, alignItems:'flex-start' }}>
          ...
          {displayTable.map(c=>(
            <AnimCard key={c.id} card={c}
              glow={glowIds.includes(c.id)}
              fade={fadeIds.includes(c.id)}
              landing={phase1Cards.find(p=>p.id===c.id) && !table.find(t=>t.id===c.id)}
              disabled
            />
          ))}
        </div>
      ...
      <div style={{ width:'100%', maxWidth:560, background:C.surface, borderRadius:14, padding:'10px 12px', border:canPlay?`1.5px solid ${C.greenLt}`:`1px solid ${C.border}`, transition:'border .3s' }}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {(hands[myIdx]||[]).map(c=>(
            <AnimCard key={c.id} card={c} onClick={()=>handleCardClick(c)} disabled={!canPlay}/>
          ))}
        </div>
      </div>

      {comboMenu&&<ComboPicker playedCard={comboMenu.card} combos={comboMenu.combos} onPick={combo=>handlePick(comboMenu.card,combo)} onSkip={()=>{ setComboMenu(null); busyRef.current=false }}/>}
