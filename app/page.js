...
function Game({ gs, myIdx, onPlayCommit, t, onLeave, isSpectator }) {
  ...
  const initiatePlay = useCallback((card) => {
    ...
    setTimeout(() => {
      ...
      if (combos.length===1) {
        glow([card.id,...combos[0].map(c=>c.id)], 850, () => onPlayCommit(card,combos[0]))
        return
      }
  ...
  const displayTable = (() => {
    let t = [...table]
    if (comboMenu && !t.find(c=>c.id===comboMenu.card.id)) t = [...t, comboMenu.card]
    if (landingId && !t.find(c=>c.id===landingId)) {
      const pendingCard = (hands[myIdx]||[]).find(c=>c.id===landingId)
      if (pendingCard) t = [...t, pendingCard]
    }
    return t
  })()
...
export default function Home() {
  ...
  const handleCreateGame = async (hostName) => {
    ...
    setWaitingNames([hostName]); setMaxPlayers(4)
  ...
  const handleJoinGame = async (name, code, setError) => {
    ...
    setWaitingNames(newNames); setMaxPlayers(4)
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      const { data } = await supabase.from('rooms').select('player_names,max_players,status,game_state').eq('id',roomId).single()
      if (!data) return
