...
function Lobby({ onCreateGame, onJoinGame, t, lang, setLang }) {
  ...
  const tabBtn = (id, label) => (
    <button onClick={() => setTab(id)} style={{
      ...
      border: tab===id ? `1.5px solid ${C.gold}` : `1px solid ${C.border}`,
      background: tab===id ? `rgba(212,168,67,.12)` : 'transparent',
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:20 }}>
      ...
      <div style={{ width:'100%', maxWidth:380, background:C.surface, borderRadius:18, padding:24, border:`1px solid ${C.borderHi}` }}>
        {tab !== 'home' && (
        ...
        {tab === 'home' && (
          <>
            <div style={{ display:'flex', gap:8, marginBottom:20 }}>
              {tabBtn('create', t.createGame)}
              {tabBtn('join', t.joinGame)}
            </div>
            <button onClick={() => setRules(true)} style={{ width:'100%', padding:'9px', borderRadius:9, border:`1px solid ${C.border}`, background:'transparent', color:C.textSub, fontSize:14, cursor:'pointer' }}>
              📖 Spielregeln
            </button>
          </>
        )}
        ...
        {tab === 'create' && (
          <>
            {/* Number of players */}
            <div style={{ color:C.textMuted, fontSize:11, letterSpacing:3, marginBottom:10, fontFamily:"'Cinzel',serif" }}>SPIELER</div>
            <div style={{ display:'flex', gap:8, marginBottom:20 }}>
              {[2,3,4].map(n => (
                <button key={n} onClick={() => setNumP(n)} style={{
                  flex:1, padding:'10px 0', borderRadius:9, fontSize:18, fontWeight:800, cursor:'pointer',
                  border: numP===n ? `2px solid ${C.gold}` : `1px solid ${C.border}`,
                  background: numP===n ? `rgba(212,168,67,.12)` : 'rgba(10,18,30,.5)',
                  color: numP===n ? C.gold : C.textSub,
                }}>{n}</button>
              ))}
            </div>

            {/* Names */}
            <div style={{ color:C.textMuted, fontSize:11, letterSpacing:3, marginBottom:10, fontFamily:"'Cinzel',serif" }}>NAMEN</div>
            {Array(numP).fill(0).map((_,i) => (
              <Input key={i} placeholder={`${t.player} ${i+1}`} value={names[i]}
                onChange={e => { const n=[...names]; n[i]=e.target.value; setNames(n) }}/>
            ))}

            {/* Team mode (only for 4 players) */}
            {numP === 4 && (
              <label style={{ display:'flex', alignItems:'center', gap:10, color:C.textSub, fontSize:14, marginBottom:18, cursor:'pointer' }}>
                <input type="checkbox" checked={team} onChange={e => setTeam(e.target.checked)} style={{ width:17, height:17, accentColor:C.gold }}/>
                {t.teamMode}
                <Tag color={C.blueLt}>2v2</Tag>
              </label>
            )}

            <Btn onClick={() => onCreateGame(
              names.slice(0,numP).map((n,i) => n.trim() || `${t.player} ${i+1}`),
              numP, team
            )}>{t.startGame}</Btn>
          </>
        )}
        ...
        {tab === 'join' && (
          <>
            <div style={{ color:C.textMuted, fontSize:11, letterSpacing:3, marginBottom:10, fontFamily:"'Cinzel',serif" }}>NAME & CODE</div>
            ...
            {joinErr && <div style={{ color:C.redLt, fontSize:12, marginBottom:14 }}>{joinErr}</div>}
            <Btn disabled={loading} onClick={async () => {
              setLoading(true)
              await onJoinGame(joinName.trim()||'Gast', joinCode, setJoinErr)
              setLoading(false)
            }}>{loading ? '…' : t.join}</Btn>

            <div style={{ marginTop:14, padding:12, borderRadius:10, background:'rgba(90,154,224,.08)', border:`1px solid rgba(90,154,224,.2)` }}>
              <div style={{ color:C.blueLt, fontSize:12, marginBottom:4, fontWeight:600 }}>💡 Zuschauer-Modus</div>
              <div style={{ color:C.textSub, fontSize:12, lineHeight:1.6 }}>Wenn das Spiel bereits läuft, trittst du automatisch als Zuschauer bei.</div>
            </div>
          </>
        )}
      </div>
    ...
    <div style={{ minHeight:'100vh', background:C.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:20 }}>
      ...
      <div style={{ width:'100%', maxWidth:400, background:C.surface, borderRadius:18, padding:26, border:`1px solid ${C.borderHi}` }}>
        ...
        <div style={{ color:C.textMuted, fontSize:11, letterSpacing:3, marginBottom:12, fontFamily:"'Cinzel',serif" }}>
          SPIELER ({playerNames.length}/{maxPlayers})
        </div>
        {Array(maxPlayers).fill(0).map((_,i) => {
          const name = playerNames[i]
          const filled = !!name
          return (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderRadius:10,
              background: filled ? 'rgba(15,25,40,.8)' : 'rgba(15,25,40,.3)',
              border: i===myIdx ? `1px solid ${C.greenLt}` : filled ? `1px solid ${C.border}` : `1px dashed ${C.textMuted}`,
              marginBottom:8, transition:'all .3s' }}>
              <div style={{ width:8, height:8, borderRadius:'50%', background: i===myIdx ? C.greenLt : filled ? C.gold : C.textMuted, transition:'background .3s' }}/>
              <span style={{ color: filled ? C.text : C.textMuted, fontSize:14, flex:1 }}>
                {filled ? name : `Warte auf Spieler ${i+1}…`}
              </span>
              {teamMode && maxPlayers===4 && filled && (
                <Tag color={i%2===0?C.gold:C.blueLt}>{`Team ${i%2===0?'A':'B'}`}</Tag>
              )}
        ...
        <div style={{ marginTop:18 }}>
          {playerNames.length < maxPlayers ? (
            <div style={{ textAlign:'center', color:C.textMuted, fontSize:13, padding:'10px 0', animation:'pulse 2s infinite' }}>
              Warte auf {maxPlayers - playerNames.length} weiteren Spieler…
            </div>
          ) : isHost ? (
            <Btn variant="green" onClick={onStart}>{t.startGame}</Btn>
          ) : (
...
export default function Home() {
  ...
  const handleCreateGame = async (playerNames, numPlayers, teamMode) => {
    const code      = genCode()
    const initialGs = initGameState(playerNames, teamMode)
    const { data, error } = await supabase.from('rooms').insert({
      code, host_name:playerNames[0], player_names:playerNames,
      max_players:numPlayers, game_state:initialGs,
      status: numPlayers===1 ? 'playing' : 'waiting',
    }).select().single()
    ...
    setGs(initialGs); subscribeRoom(data.id)
    setScreen(numPlayers===1 ? 'game' : 'waiting')
  }
  ...
  const handleStartGame = async () => {
    ...
    const newGs = initGameState(data.player_names, gs.teamMode)
